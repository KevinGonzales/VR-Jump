...
public class InstantiateTIlesForLevelBased : MonoBehaviour {
	...
	void Start () {
		...
		int length = GameControllerForGameWithLevel.Instance.getLengthForTile ();
		int width = GameControllerForGameWithLevel.Instance.getWidthForTiles ();
	...
	private int itemToInstantiate(){
		...
		if(num <= GameControllerForGameWithLevel.Instance.getChanceForPowerUp()){
				return 2;
		}
		else if (GameControllerForGameWithLevel.Instance.getChanceForPowerUp () < num && num <= (GameControllerForGameWithLevel.Instance.getChanceForHindrence () + GameControllerForGameWithLevel.Instance.getChanceForPowerUp ())) {
				return 1;
		} else {
			return 0;
		}
